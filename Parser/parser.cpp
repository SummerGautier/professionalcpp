...
double EvaluateExpression (TokenStream& _tokenStreamRef) {
    ...
    double _left = EvaluateTerm(_tokenStreamRef);
    ...
    while (true) {
        switch (_token.kind) {
            ...
            case '+':
                _left = _left + EvaluateTerm(_tokenStreamRef);
...
double EvaluateTerm (TokenStream& _tokenStreamRef) {
    ...
    double _left = EvaluatePrimary(_tokenStreamRef);
    ...
    while (true) {
        switch (_token.kind) {
            ...
            case '*':
                ...
                _left = _left * EvaluatePrimary(_tokenStreamRef);
                // update token.
                _token = _tokenStreamRef.GetToken();
                break;
            ...
            case '/':
                { // the curly brace 'block' is necessary since we define & init a variable (primary)
                    double _divisor = EvaluatePrimary(_tokenStreamRef);
...
double EvaluatePrimary(TokenStream& _tokenStreamRef) {
    ...
    switch (_token.kind) {
        case '(':
            {
                double _value = EvaluateExpression(_tokenStreamRef);
                _token = _tokenStreamRef.GetToken();
                if (_token.kind != ')') {
